...
class AppState:
    def __init__(self):
        ...
        # Remove: self.r_script_path = None
...
def visualize_multigraph(adjacency_path, output_path, scale_factor=1.0):
    """
    ...
    try:
        ...
        if not os.path.exists(adjacency_path):
            print(f"Error: Input file does not exist: {adjacency_path}")
            return False
        
        # Read the adjacency list CSV data
        adjacency_data = pd.read_csv(adjacency_path)
        
        # Parse adjacency string to array of node IDs
        ...
        plt.title(f"Multigraph from {os.path.basename(adjacency_path)}", fontsize=14)
        plt.figtext(
            0.5, 0.01,
...
def visualize_multigraph_pygraphviz_simple(adjacency_path, output_path, scale_factor=1.0):
    """
    Simplified multigraph visualization with PyGraphviz, no legend.
    """
    try:
        ...
        G.graph_attr.update({
            'overlap': 'false',
            'splines': 'true',
            'K': f"{1.0/scale_factor}",
            'size': f"{12*scale_factor},{9*scale_factor}",
            'dpi': '300'
        })
        
        # Add nodes and track degrees
        node_degrees = {}
        for _, row in adjacency_data.iterrows():
            node_id = row['node']
            adj_list = parse_adjacencies(row['adjacencies'])
            node_degrees[node_id] = len(adj_list)
            
            # Add node with color based on degree
            if len(adj_list) == 1:
                color = 'lightblue'  # Endpoint
            elif len(adj_list) >= 3:
                color = 'red'        # Junction
            else:
                color = 'lightblue'  # Other
            
            G.add_node(node_id, shape='circle', style='filled', fillcolor=color)
        ...
        # Set title with basic stats
        title = f"Multigraph from {os.path.basename(adjacency_path)}"
        stats = f"Nodes: {len(node_degrees)} - Edges: {len(edge_count)}"
        G.graph_attr['label'] = f"{title}\n\n{stats}"
        
        # Choose layout algorithm
        layout_prog = 'neato'  # Generally good for network layouts
...
def visualize_multigraph_pygraphviz(adjacency_path, output_path, scale_factor=1.0):
    """
    Visualize a multigraph from an adjacency list CSV file using PyGraphviz.
    ...
    try:
        ...
        # Build a NetworkX graph to analyze the structure
        import networkx as nx
        nx_G = nx.Graph()
        
        # Add all nodes with their component info
        for _, row in adjacency_data.iterrows():
            node_id = row['node']
        ...
        # Add edges
        edge_count = {}  # Track edge multiplicity
        ...
        for _, row in adjacency_data.iterrows():
            ...
            for neighbor in adj_list:
                nx_G.add_edge(node_id, neighbor)
                # Track edge multiplicity
                edge_key = tuple(sorted([node_id, neighbor]))
                if edge_key in edge_count:
                    edge_count[edge_key] += 1
                else:
        ...
        # Analyze the graph to choose the best layout
        num_nodes = nx_G.number_of_nodes()
        is_planar = nx.check_planarity(nx_G)[0]
        density = nx_G.number_of_edges() / (num_nodes * (num_nodes - 1) / 2) if num_nodes > 1 else 0
        
        # Choose layout algorithm based on graph properties
        if is_planar:
            layout_prog = 'fdp'  # fdp is good for planar graphs
        elif density < 0.2:
            layout_prog = 'neato'  # neato works well for sparse graphs
        else:
            layout_prog = 'sfdp'  # sfdp for denser graphs
            
        print(f"Using layout algorithm: {layout_prog}")
        ...
        G.graph_attr.update({
            ...
            'sep': '+15',          # Increase separation between nodes
            'K': f"{1.0/scale_factor}",  # Ideal edge length
            'pack': 'true',        # Pack disconnected components
            'mclimit': '2.0',      # Emphasize reducing edge crossings
            'size': f"{12*scale_factor},{9*scale_factor}",  # Size in inches
        ...
        for node_id in nx_G.nodes():
            ...
            if node_degrees[node_id] == 1:
                fillcolor = 'lightblue'  # Endpoints
            elif node_degrees[node_id] >= 3:
                fillcolor = 'red'        # Junctions
            else:
                fillcolor = 'lightblue'  # Other nodes
            
            # Calculate node size based on scale
            node_size = max(0.1, 0.3 / scale_factor)
        ...
        processed_edges = set()
        max_multiplicity = max(edge_count.values()) if edge_count else 1
        
        for node_id in nx_G.nodes():
            for neighbor in nx_G.neighbors(node_id):
                edge_key = tuple(sorted([node_id, neighbor]))
                ...
                multiplicity = edge_count[edge_key]
                penwidth = max(0.5, (1.0 + 0.5 * multiplicity/max_multiplicity) / scale_factor)
                
                # For multiple edges, handle differently
                if multiplicity > 1:
                    # Add multiple edges that will be curved automatically
                    for i in range(multiplicity):
                        G.add_edge(node_id, neighbor, penwidth=penwidth, color='gray')
                else:
                    # Single edge
                    G.add_edge(node_id, neighbor, penwidth=penwidth, color='gray')
        
        # Add legend node with HTML-like label
        legend_label = '<TABLE BORDER="0" CELLBORDER="1" CELLSPACING="0" CELLPADDING="4">'
        legend_label += '<TR><TD COLSPAN="2"><B>Legend</B></TD></TR>'
        legend_label += '<TR><TD BGCOLOR="lightblue">Endpoints (deg 1)</TD></TR>'
        legend_label += '<TR><TD BGCOLOR="red">Junctions (deg 3+)</TD></TR>'
        legend_label += '<TR><TD BGCOLOR="lightblue">Other Nodes</TD></TR>'
        legend_label += '</TABLE>'
        
        G.add_node('legend', shape='none', label=legend_label)
        
        # Calculate statistics for footer
        total_nodes = len(nx_G.nodes())
        unique_edges = len(edge_count)
        total_edges = sum(edge_count.values())
        multiple_edges = sum(1 for count in edge_count.values() if count > 1)
        
        # Create label with statistics
        stats_label = f"Total nodes: {total_nodes} - Unique edges: {unique_edges} - "
        stats_label += f"Total connections: {total_edges} - "
        stats_label += f"Edges with multiple connections: {multiple_edges} - "
        stats_label += f"Scale: {scale_factor}x"
        
        # Set title and footer
        title = f"Multigraph from {os.path.basename(adjacency_path)}"
        G.graph_attr['label'] = f"{title}\n\n{stats_label}"
        G.graph_attr['labelloc'] = 't'  # Position at top
        G.graph_attr['fontsize'] = '14'
...
def visualize_adjacency_graph(adjacency_path, output_path, scale_factor=1.0):
    """
    Wrapper function to visualize a graph from an adjacency list CSV.
    ...
    Returns:
        bool: True if visualization succeeded, False otherwise
    """
    return visualize_multigraph(adjacency_path, output_path, scale_factor)

def open_graph_in_new_window(graph_image_path):
    """
    Open the graph visualization in a new Napari viewer window.
    
    Args:
        graph_image_path (str): Path to the graph image file
    
    Returns:
        napari.Viewer: The new viewer instance
    """
    try:
        ...
        graph_image = io.imread(graph_image_path)
        
        # Create a new viewer
        graph_viewer = napari.Viewer(title="Network Graph Visualization")
        
        # Add the image to the viewer
        graph_viewer.add_image(
            graph_image,
            name='Network Graph',
        )
        
        show_info(f"Graph opened in new Napari window")
        return graph_viewer
        
    except Exception as e:
        show_error(f"Error opening graph in new window: {str(e)}")
        return None
...
class FileLoaderWidget(QWidget):
    ...
    def __init__(self, viewer):
        super().__init__()
        self.viewer = viewer
        self.graph_viewer = None  # Add this line to store reference to graph viewer
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        network_layout.addWidget(self.open_graph_btn)
    ...
    def on_network_clicked(self):
        ...
        try:
            ...
            if adjacency_path and edge_path:
                ...
                # Enable graph visualization button directly
    ...
    def on_view_clicked(self):
        ...
        try:
            ...
            if raw_im is not None and skel_im is not None:
                ...
                # Enable graph visualization if adjacency list exists
                if app_state.adjacency_path and os.path.exists(app_state.adjacency_path):
                    self.graph_btn.setEnabled(True)
    ...
    def on_graph_clicked(self):
        """Handle graph button click to visualize network as a graph in the widget panel."""
        ...
        try:
            ...
            # Generate graph visualization using Python
            self.log_status(f"Visualizing graph using Python (scale factor: {scale_factor})...")
            success = visualize_adjacency_graph(app_state.adjacency_path, app_state.graph_image_path, scale_factor)
            ...
            if success:
                ...
                available_width = self.graph_display.width() - 20  # Subtract some padding
                ...
                self.graph_display.setMinimumSize(scaled_pixmap.width(), scaled_pixmap.height())
                
                self.log_status("Graph visualization displayed in widget")
    ...
    def setup_ui(self):
        """Create the user interface."""
        # Create the main layout first and set it
        layout = QVBoxLayout()
        self.setLayout(layout)
        ...
        # ... all your other existing UI elements ...
        # File selection section
        file_group = QGroupBox("File Selection")
        file_layout = QVBoxLayout()
        file_group.setLayout(file_layout)
        ...
        graph_options_layout.addRow("Scale Factor:", self.scale_factor_spin)

        layout.addWidget(graph_options_group)
        ...
        # NEW: Add button for opening graph in new window
        self.open_graph_btn = QPushButton("Open Graph in New Window")
        self.open_graph_btn.clicked.connect(self.on_open_graph_window_clicked)
        self.open_graph_btn.setEnabled(False)
        network_layout.addWidget(self.open_graph_btn)
        ...
        # Create a section for graph visualization
        graph_group = QGroupBox("Graph Visualization")
        graph_layout = QVBoxLayout()
        graph_group.setLayout(graph_layout)
        
        # Create a scroll area to contain the graph image
        self.graph_scroll = QScrollArea()
        self.graph_scroll.setWidgetResizable(True)
        self.graph_scroll.setMinimumHeight(300)
        
        # Create a label to display the graph image
        self.graph_display = QLabel("No graph generated yet")
        self.graph_display.setAlignment(Qt.AlignCenter)
        self.graph_display.setMinimumSize(300, 300)
        
        # Add the label to the scroll area
        self.graph_scroll.setWidget(self.graph_display)
        
        # Add the scroll area to the graph layout
        graph_layout.addWidget(self.graph_scroll)
        
        # Add the graph section to the main layout
        layout.addWidget(graph_group)


    def on_open_graph_window_clicked(self):
        """Handle click on button to open graph in new window."""
        if not app_state.graph_image_path or not os.path.exists(app_state.graph_image_path):
            self.log_status("No graph image available. Please visualize the graph first.")
            return
        
        self.log_status("Opening graph in new Napari window...")
        self.graph_viewer = open_graph_in_new_window(app_state.graph_image_path)
        
        if self.graph_viewer is not None:
            self.log_status("Graph opened in new window successfully")
        else:
            self.log_status("Failed to open graph in new window")
...
class AdjacencyGraphViewer(QWidget):
    ...
    def setup_ui(self):
        ...
        self.scale_factor_spin.setSingleStep(0.1)
        self.scale_factor_spin.setDecimals(1)
        options_layout.addRow("Scale Factor:", self.scale_factor_spin)
